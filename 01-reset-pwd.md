...
> Были удалены все лишние ключи из строки linnux16 в GRUB2, которые мешали загрузке ядра в `emergence mode` кроме тех, которые указывают, 
> где лежат разделы на `lvm` и файл ядра `Linux`, не тронут ключ `ro` и добавлен `init=/bin/bash`
